...
The verification handoff distinguishes the imported component checkpoint from subsequent full runtime integration; its retired dated record is described by `$AGENT_ARCHITECTURE_DIR/archived/README.md` and active stage records are registered in `$AGENT_ARCHITECTURE_DIR/docs/plans/pi-integration/README.md`. Authored source changes do not update an installed local package snapshot automatically.
