...
- the panel is padded and painted with the `selectedBg` background: an accent title row (`Subagents · <phase>`), a muted rule, a summary row, one row per task, and a dim key footer; it fits the widest observed row above an 80-column floor, grows when new observations need more room, and never fills the terminal
