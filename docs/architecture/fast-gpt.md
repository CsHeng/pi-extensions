...
The `fast-gpt` status reports the selected request profile, not a confirmed provider result. Standard OpenAI Responses payloads expose the actual tier as top-level `response.service_tier`, including under `response.completed.response.service_tier` in the streaming protocol, but Pi's extension hooks expose only response status and headers after dispatch. Pi's normalized assistant message does not retain that field, so this standalone extension cannot inspect or display the tier actually served.
