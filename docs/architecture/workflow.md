...
Version two replaces the normal `open/start/record/assess/batch` recipe with semantic operations. Version-one reducer, validation and host fixtures remain compatibility evidence; the package entry point does not register their old model protocol or automatic-review policy. An already running Pi process keeps the package version it loaded until explicitly restarted/reloaded.
...
| `inspect` | Read bounded current requirements, tasks, facts and judgments. Optional `subject` selects `requirement:<key>`, `task:<key>`, an attempt ID or a fact ID. Legacy state is readable without promotion. |
...
Fulfillment (`pending`, `complete`, `cancelled`, `superseded`) is independent of continuation (`active`, `waiting`, `suspended`). Waiting on a real authority, decision, prerequisite, capability, conflict or non-convergence condition does not satisfy the goal. A task blocker names both the reason and unblock condition. Independent ready work remains active; continuation waits only when the remaining tasks cannot advance. `resume` can clear one named task's resolved blocker or explicitly resume the whole contract.
...
The newest active-branch snapshot is authoritative. Malformed, counter-inconsistent or unsupported state is unavailable, with no fallback to an earlier accepted snapshot. A valid v1 snapshot is read-only legacy state and never auto-promoted. Explicit same-goal `enroll` with `migrateLegacy: true` must preserve all legacy required outcomes/verifications and stores legacy identity/revision provenance; prior acceptance is not fresh proof. This uses existing authority, not an invented new permission requirement. An old reader refuses a newest v2 snapshot. Restart/tree replacement interrupts copied running attempts, clears ephemeral observations and requires explicit alignment/resume, never automatic work restart. Removing the extension leaves inert history.

## Optional projection

The TUI view is off by default. `/workflow-ui` toggles it; `show` and `hide` control visibility, and `list` uses the host's scrollable read-only selection view. Rows show task titles and distinguish pending, running and explicitly accepted work. Width clipping is display-only. Controls do not enroll, change acceptance, resume work or supply model context. RPC, print and JSON mount no view. There is no footer or working-message writer; UI failure disables only the view.

## Verification boundary

`tests/workflow-goal.test.ts` owns semantic transitions, evidence/drift, migration, persistence, suspension and pure projection invariance. `tests/workflow-goal-ui.test.ts` checks optional view controls, headless inactivity and failure isolation. `tests/workflow-goal-host.test.ts` owns real-host registration, schema, ordinary-work controls, check-time basis, queue preparation, repeated no-progress settlement, and three productive continuations through fulfillment on unchanged source. Co-load and installed-CLI synthetic-provider fixtures exercise the default v2 entry alongside subagents, observer, timing and footer. Retained v1 tests import an explicitly named historical fixture and do not certify the new runtime. Both workflow package probes use semantic enrollment with disposable settings and a synthetic provider.

The supported offline host is Pi 0.85.1. These tests establish host/protocol correctness, not real-model requirement completeness, total inference savings or reduced task abandonment. Publication to the copied daily package, user restart/load and bounded live effectiveness evaluation remain separate authorized actions. See the [approved implementation plan](../plans/changes/2026-09-18-goal-driven-workflow-plan.md) for source-delivery evidence and milestone limits.
