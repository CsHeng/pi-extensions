...
Runtime telemetry schema four retains schema-three wall start time, opaque extension and configuration epochs when provenance is available, and effective concurrency caps, and adds explicit monotonic timing evidence. `$evaluate-subagent-runs` accepts one explicit session and can, with an explicit sessions root and provenance manifest, select current-epoch schema-three and schema-four runs. Plan eligibility remains unavailable without separately approved plan evidence.
