...
Runtime telemetry schema three records start time, opaque extension and configuration epochs when provenance is available, and the effective concurrency caps actually used. `$evaluate-subagent-runs` still accepts one explicit session and can, with an explicit sessions root and provenance manifest, select only current-epoch schema-three runs. Plan eligibility remains unavailable without separately approved plan evidence.
...
Ordinary tasks omit `dependsOn` and run as a flat batch. Hard edges represent only approved implementation order with no intervening parent decision; the extension validates mechanics but never detects an active Skill. The tool does not accept arbitrary working directories, tool lists, commands, extensions, Skills, retries, background flags, or nested graphs. Model-facing role, execution-profile, reasoning-profile, and thinking choices serialize as direct string enums where bounded through Pi's provider-compatible `StringEnum` helper. Worker write files are never inferred. A worker declaration of `externalReadRoots`, including an explicit empty array, is rejected.
...
The structured aggregate result records per-task status, effective route and profile decision, bounded output, usage, duration, changed paths, convergence state, stop reason, relative diagnostic reference, bounded activity, and typed error. Version-two redacted telemetry adds an invocation-local run ID, requested/admitted/launched counts, requested/admitted hard-edge counts, explicit model/thinking task counts, run duration, peak global and role concurrency, route selection source, structured run errors, and task queue/workspace/child/convergence durations. It remains inside Pi's persisted tool result and stores no path or repository counter. Path-admission failures report requested counts, zero admitted tasks, zero launches, and no raw path text. Retained child sessions form a second evidence ledger, not an execution ledger; they do not recover graph, scheduler, workspace, lock, convergence, or continuation state. Child prose and session entries cannot modify graph state.
...
The extension has no background execution, durable graph or orchestration ledger, replay, semantic resume, mission state, approval protocol, review gate, repair loop, generic permission framework, external-file write, child command runner, automatic model retry, dynamic role, or Skill-loading bridge. Explorer and reviewer external reads do not imply multi-repository workers, another repository as child cwd, persistent repository aliases, or per-task permission prompts; those remain future designs. Diagnostic process settlement and retained standard Pi sessions do not grant continuation or convergence authority. It does not install another package or mutate Pi settings.
