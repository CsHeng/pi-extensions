...
The default guidance is `aggressive`: it always states the batch shape and adds the measured trigger that a request needing evidence from three or more independent files, directories, modules or subsystems should submit one create batch with one explorer task per slice before reading them serially, then synthesize the reports. `balanced` states the batch shape without that trigger, and `off` injects no guidance; the tool snippet and tool guidelines registered with the tool itself remain the same at every level. A singleton remains valid for a required isolated worker or one independent reviewer, not ordinary parent-work offload. A single episode may complete the task; continuation is explicit. Parent owns synthesis, verification, acceptance, apply, and close. File-dependent successors require explicit parent apply between dispatches; dependency edges pass reports, not candidate files. Guidance does not delegate trivial work or parent-owned synthesis, verification, authority, adjudication, repair decisions, continuation, or the final response. A batch must stop before any such parent decision point; later work uses another parent tool call.
...
When the user explicitly names a model or exact thinking level, the parent may pass task `model` and `thinking` for explorer, reviewer, or worker. Explicit `model` supersedes role candidates and execution-profile model mapping; role still owns tools, paths, concurrency, isolation, and candidate export. Explicit `thinking` supersedes `reasoningProfile`; with a default route it may select the first ordered candidate supporting that exact level, while an explicit model never falls back to another model or level. An explicit model without a task reasoning override inherits the parent turn's exact thinking level.
...
All route resolution uses the full public `getAll()` catalogue and authenticated `getAvailable()` registry snapshot, not the session's `scopedModels` cycling shortcuts. Cycling membership and cycling-specific thinking pins do not restrict default, parent-inherited, semantic-profile, or explicit routes. Default resolution retains ordered candidates, the parent model/current thinking where requested, model availability, and thinking support; when none qualifies it returns `route_unavailable`. Explicit model resolution takes the same registry snapshots. It tries case-insensitive exact `provider/model`, normalized exact bare ID, then normalized exact display name; normalization applies Unicode NFKC, lowercase, punctuation-run collapse, and trim. It never uses partial or catalogue-order matching. Missing, unavailable, ambiguous, and unsupported-thinking choices return typed `model_not_found`, `model_unavailable`, `ambiguous_model`, or `thinking_unavailable` failures before child launch. Ambiguous and unavailable diagnostics retain at most eight sorted canonical candidates.
...
The `scripts/run-delegation-probe.ts` lane measures a different question: whether a parent model calls the registered tool at all, and how the parent-visible surface changes that decision. It runs the installed `pi` against a throwaway project and one mock tool from `scripts/fixtures/delegation-probe-extension.ts`, which imports `SUBAGENT_TOOL_*` and the configured guidance lines instead of restating them, so a variant cannot drift from shipped text. Shapes cover the production schema, a flat create schema derived from the production task fields, and one optional external `Agent` reference shape; guidance levels mirror `off`, `balanced`, and `aggressive`. The lane always includes control prompts, prints Wilson intervals, records a frozen prompt-set fingerprint and method fingerprint, and writes one JSON artifact; it is provider-gated by `CSHENG_SUBAGENTS_DELEGATION_PROBE=1`, never part of `bun run test`, and never a pass or fail gate. Its mock executor provides no child report, so it measures a call decision rather than child quality, and its dated results belong in `$AGENT_ARCHITECTURE_DIR/docs/evaluations/pi-integration/` rather than in this repository.
