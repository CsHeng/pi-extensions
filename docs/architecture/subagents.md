...
Live TUI tool progress uses the run clock rather than the longest child. In TUI mode the core also publishes bounded `csheng.subagents.observer.v2` snapshots with launched/running/finished counts, aggregate assistant turns, elapsed time, and per-task model/thinking/turns/elapsed facts. Default-loaded `subagents-ui` consumes those snapshots as a read-only overlay; see [Subagents UI](subagents-ui.md). `status-footer` and `work-timing` remain independent.
