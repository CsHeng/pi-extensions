...
| F06 subagents: execution | `csheng_subagent_sessions` | Session-owned asynchronous native execution, linked Git worktrees, candidates | Accepted receipts are not completion; shared capacity, explicit refresh/apply/cancel/close; child tools are not parent tool events; no automatic acceptance | `P/subagents/{index,continuation}.ts` |
...
| F08 subagents: publication | `pi.events` observer snapshots and persisted v3 terminal events | Observer and execution evidence buses | Persist before coalesced current-owner wake; no completion judgment | `P/subagents/{continuation,observation-hooks}.ts` |
...
| F16 workflow: evidence | `tool_execution_start/end`, v3 execution events | Check-start basis and dispatch-bound terminal evidence | Receipts are not completed checks; inspect/join preserve original contract/attempt/input; fusion is one compound observation | `P/workflow/goal-host.ts`: `captures`; `observation.ts` |
...
| Complex search, structured processing, quoting, COUNT/PREVIEW/EXECUTE judgment | Authored portable Skill source; regenerate its distribution through its owning repository | Editing generated Skills or copying semantic rules wholesale into Pi hooks |
