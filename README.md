...
The optional TUI projection is off by default. `/workflow-ui` toggles it, `hide` and `show` control visibility, and `list` opens a host read-only selection view. Rows distinguish pending, running and accepted tasks; clipping is display-only. View controls never enroll, mutate or complete a contract. RPC/JSON/print modes mount no UI.
