...
This repository contains small, independently removable Pi extensions. It exports nine maintained extensions: `plan-mode`, `multi-skill-mentions`, `fast-gpt`, `subagents`, `subagents-ui`, `herdr-handoff`, `status-footer`, `work-timing`, and `workflow`, while preserving Pi's authoritative host agent loop. `subagents-ui` is default-loaded as a TUI-only read-only observer.
...
The TUI projection is enabled by default and appears once a contract, legacy snapshot or recovery diagnostic exists; an empty branch shows no widget. Manual hide survives updates, and a fresh extension load restores the default. `/workflow-ui` toggles it, `hide` and `show` control visibility, and `list` opens a host read-only selection view. The above-editor view uses a todo-style `● Tasks · <goal> (done/total)` heading and themed tree rows: pending `○`, running `◐`, blocked `!`, and accepted `✓` with dimmed strikethrough titles. It does not display internal contract, continuation or input receipts. The widget caps at 120 columns and one third of terminal height (at most 12 content rows), prioritizes active/blocked tasks on overflow, and keeps task titles ahead of long keys or hints; clipping is display-only. View controls never enroll, mutate or complete a contract. RPC/JSON/print modes mount no UI.

State lives only in bounded schema-v2 `csheng-workflow-state` session snapshots, appended before installation in memory. Corrupt or unsupported newest state fails closed without fallback. Valid v1 history remains readable but requires explicit same-goal re-enrollment preserving requirements; old acceptance is not fresh proof. Restart and branch changes interrupt running attempts and require explicit alignment/resume. There is no extra state file, automatic recovery, footer or working-message writer. Removing the extension leaves inert history. See [`docs/architecture/workflow.md`](docs/architecture/workflow.md) for protocol, evidence, continuation, migration and verification limits. Source correctness does not prove live-model effectiveness or update an already running daily Pi process.
