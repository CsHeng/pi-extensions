...
Live tool progress uses `Subagents {running}/{total} running, {finished} finished · {turns} turns · {clock}`. `subagents-ui` is an optional one-shot TUI consumer for keyed status, a below-editor panel, and `/subagents-ui` inspection; it is not in the default package load list. Managed episodes use the native inline tool-update channel and bounded result rendering that shows the actual provider/model/thinking, request and episode outcomes, stored state, report completeness, and apply facts. Acceptance remains a parent judgment, not a runtime result field or warning.
