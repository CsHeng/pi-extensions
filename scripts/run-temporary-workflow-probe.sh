...
rpc_output=$(
	...
	printf '%s\n' \
		...
		CSHENG_PROBE_PACKAGE_ROOT="${repo_root}" PI_CODING_AGENT_DIR="${agent_root}" PI_OFFLINE=1 pi \
			...
			--no-approve \
			--no-extensions \
			--extension "${repo_root}/extensions/workflow/index.ts" \
...
coload_output=$(
	...
	printf '%s\n' \
		...
		PI_CODING_AGENT_DIR="${agent_root}" PI_OFFLINE=1 pi \
			...
			--extension "${repo_root}/extensions/work-timing/index.ts" \
			--extension "${repo_root}/extensions/status-footer/index.ts" \
...
jq -cn '{result:"pass",source:"temporary",tool:1,source_identity:1,extension_off_tool:0,coload:1,synthetic_snapshots:1}'
