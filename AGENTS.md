...
- The status-footer extension may replace the TUI footer with a compact status that renders as one line on wide terminals and wraps field by field when the single line would not fit — each field flows onto the next row only when it no longer fits the remaining whitespace, adding rows rather than dropping fields: model and thinking, an optional warning-colored ⚡ after thinking when Pi's keyed `fast-gpt` status is present, optional `(provider sub)` when the active model is on a subscription, the session's original workdir with the Pi-provided git branch, the full session UUID, session-cumulative ↑ input, ↓ output, R cache-read, optional W cache-write, latest-prompt CH hit rate, cost, current context fill, and `MCP enabled/all` from the mcp-adapter status bus. It must keep the UUID visible when width is tight even after wrapping, truncating a row only when a lone segment cannot fit, omit generic extension-status chrome, remain inactive in RPC, JSON, and print modes, and must not persist session metadata. It reads the branch and keyed statuses from Pi's footer data provider without importing `fast-gpt`, spawning Git, or resolving worktree layout, and does not own Git state or chrome, working-message timing, provider billing truth, or the request-profile selection.
- The work-timing extension may customize the TUI working message while an interaction is active and append one context-free completion entry after `agent_settled`. Reasoning duration is client-observed time between streamed thinking start and end events; total duration spans `before_agent_start` through final settlement. The working label reports the current user message with `Working... 12s • ↑ 17,234 ↓ 4,321 tokens • R 3s / ΣR 16s • 720 tok/s`: the elapsed clock and the upstream/downstream token counts accumulate across every turn and tool call that message triggered and therefore never disappear while a tool runs, `R` is the current turn's reasoning, and `ΣR` the message's cumulative reasoning. While a tool has been executing for at least five seconds, the live label inserts `$ <tool> <elapsed>` right after the head clock, as in `Working... 47m 29s • $ bash 46m 51s • ↑ 9,861 ↓ 20,323 tokens • R 4s / ΣR 1m 33s • 217 tok/s`: the longest-running active tool leads, `+N` counts the other concurrent tools, and time spent waiting on extension UI prompts is excluded from that tool clock. The settled entry keeps that field order and is fully cumulative for the message: `Worked for 12s • ↑ 17,234 ↓ 4,321 tokens • ΣR 16s (45%) • 720 tok/s`. `↑` comes from provider `usage.input`; `↓` prefers provider `usage.output` and falls back to the chars/4 estimate of streamed text, thinking, and tool-call arguments; an unreported side is omitted. `tok/s` is decode throughput: output tokens from turns that actually streamed, divided by first-token-to-pause time. Each turn starts the decode clock on `thinking_start` or the first `thinking_delta` / `text_delta` / `toolcall_delta`, then runs through thinking and stream stalls until assistant `message_end` or the first `tool_execution_start`. TTFT, tool execution, usage-only turns, and the gap between turns do not count. It disappears while decode time or streamed output tokens are still zero. The footer keeps its own session-cumulative counters. Use two refresh lanes with one working-line formatter and writer: the one-second clock samples elapsed time, `R`, and `ΣR` together; stream events refresh tokens and `tok/s` immediately against that cached duration sample. Lifecycle edges may refresh the full sample, and settlement renders final values once. Both lanes always include the same `R` / `ΣR` structure, never separate short and full working messages; unchanged text is not republished. The live label compacts to the host's single-line working row — fullscreen renders only a message's first line — using the host's width source (`process.stdout.columns`, then `COLUMNS`, then 80) minus the loader's two-column padding and the spinner's two leading columns: fields keep their order, `R / ΣR` always stays together with the live tool field, and fields that no longer fit are omitted whole instead of truncated, repainting immediately on terminal resize. The settled entry keeps every field and folds at ` • ` field boundaries only. Changing refresh frequency does not arbitrate competing extensions: when co-loaded with pi-cc-extensions, its independent `enableWorkingMessage` feature must be off. Never change another extension's settings or patch Pi's UI methods from runtime code to claim the row. The offline installed-host regression in `tests/work-timing-tui.test.ts` exercises the actual Pi/CC TUI with that writer off and disposable settings; it does not establish which writer was loaded in an existing user process. Collapsed and expanded states render that same entry line, version-one entries render without the cumulative token segment, and a non-zero sub-second reasoning duration renders `<1s` instead of a share-contradicting `0s`. It must clean up its interval on settlement and session shutdown and remain inactive in headless modes.
