...
export const SUBAGENT_TOOL_DESCRIPTION = "Submit bounded session-owned asynchronous tasks; accepted receipts are not completed work. Workers inherit a fixed Git input in an owned linked worktree; initial write regions are advisory. Inspect/join terminal evidence, explicitly apply Git candidates, refresh idle input, cancel, continue or close. Trusted host tools are not an OS sandbox.";
...
export const SUBAGENT_TOOL_PROMPT_GUIDELINES: readonly string[] = [
	...
	"Workers require scope [\".\"]. Git candidates contain actual changes, not only initial writePaths. Refresh input explicitly while idle; continue retains the same input, worktree and native history. No automatic apply, acceptance, retry or recovery.",
